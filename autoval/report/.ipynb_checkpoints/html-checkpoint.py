...
def singleReport (cfg, tag, info, datespan, stats, avgStats):
    ...
    # Initialize dictionaries to store separate stats for averaging
    nos_stats_list = []
    ioc_stats_list = []
    
    # Loop through all stations to separate stats
    for n in range(len(ids)):
        current_stats = stats[n]
        current_state = states[n]
        current_country = countries[n]
        
        # Determine if it's an NOS station (has a US state and not a country)
    ...
    if nos_stats_list:
        for key in skill_keys:
            # Gather all non-NaN values for this skill metric from NOS stations
            values = [s.get(key, np.nan) for s in nos_stats_list if not np.isnan(s.get(key, np.nan))]
            if values:
                avgStats_nos[key] = np.mean(values)
            else:
                avgStats_nos[key] = np.nan
    
    avgStats_ioc = {}
    if ioc_stats_list:
        for key in skill_keys:
            # Gather all non-NaN values for this skill metric from IOC stations
            values = [s.get(key, np.nan) for s in ioc_stats_list if not np.isnan(s.get(key, np.nan))]
            if values:
                avgStats_ioc[key] = np.mean(values)
            else:
    ...
    with open(local,"r") as fid:
        for line in fid:
            if '<!--InsertDate-->' in line:
                ...
                fod.write('<h2>Average time-series statistics for NOS and IOC Stations:</h2>\n')
                ...
                # Table 1: NOS Stations

                csv2html (fod, avgStats)
                
                if avgStats_nos:
                    fod.write('<h2>Average time-series statistics for NOS Stations:</h2>\n')
                    csv2html (fod, avgStats_nos)
                    fod.write('<br>\n') # Add a line break for separation
                
                # Table 2: IOC Stations
                if avgStats_ioc:
                    fod.write('<h2>Average time-series statistics for IOC Stations:</h2>\n')
